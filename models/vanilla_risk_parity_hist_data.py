...
if __name__ == '__main__':
    ...
    for assetLabel in assetLabels:
        ...
        for column in ["open", "high", "low", "close"]:
            df[column] = add_leverage(df[column], leverage=1, expense_ratio=0.01)
        data.append(
            bt.feeds.PandasData(dataname=df, fromdate=start, todate=end, timeframe=bt.TimeFrame.Days, compression=1))
    ...
    print('Stddev (annualized from monthly returns):%0.3f' %(stddev*np.sqrt(12)*100))
    print('Sharpe Ratio (annualized from monthly excess return):%0.3f' %(sharpe*np.sqrt(12)))
